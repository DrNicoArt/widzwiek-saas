...
export async function transcribeLocally(file: File, onProgress?: ProgressFn): Promise<CaptionDocument> {
  ...
  // eslint-disable-next-line
  const lang = getAsrLang();
  const out: any = await transcriber(audio, {
    ...(lang.whisper ? { language: lang.whisper } : {}),
    task: "transcribe",
  ...
  const doc: CaptionDocument = {
    schema_version: "1.0",
    media: { filename: file.name, source_kind: file.type.startsWith("video") ? "video" : "audio", duration_ms: duration, language: lang.code },
    speakers: [], cues,
